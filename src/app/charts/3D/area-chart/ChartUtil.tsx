...
const ChartUtil = ({ data }: ChartProps) => {
  ...
  const renderSVG = useCallback(() => {
    ...
    self.chart = self.svg.append('g')
      .attr('transform', `translate(${self.margin.left + (zAngle > 90 ? Math.abs(xShift) : 0)}, ${self.margin.top + yShift})`);
  ...
  const setScales = useCallback(() => {
    ...
    const domainMax: any = d3.max(data, (d: { value: number; }) => Math.ceil(d.value / 10) * 11);
  ...
  const renderArea = useCallback(() => {
    ...
    const spacer = 0;
    // render top surface
    const topSurfaceContainer = areaPathContainer.append('g')
      .attr('class', 'top-surface');
    for (let i = 0; i < data.length - 1; i++) {
      const d1 = data[i];
      const d2 = data[i+1];

      const [dx1, dx2] = [self.xScale(d1.focusGroup) + self.xScale.bandwidth() / 2 - spacer, self.xScale(d1.focusGroup) + xShift + self.xScale.bandwidth() / 2 - spacer];
      const [dy1, dy2] = [self.yScale(d1.lineValue), self.yScale(d1.lineValue) - yShift];

      const [dx3, dx4] = [self.xScale(d2.focusGroup) + xShift + self.xScale.bandwidth() / 2 + spacer, self.xScale(d2.focusGroup) + self.xScale.bandwidth() / 2 + spacer];
      const [dy3, dy4] = [self.yScale(d2.lineValue) - yShift, self.yScale(d2.lineValue)];

      const polyString = [[dx1, dy1].join(','), [dx2, dy2].join(','), [dx3, dy3].join(','), [dx4, dy4].join(',')].join(' ');
      topSurfaceContainer
        .append('polygon')
    ...
    topSurfaceContainer
        .append('polygon')
        .attr('points', `${self.xScale(data[0].focusGroup) + self.xScale.bandwidth() / 2},${self.height} 
          ${self.xScale(data[0].focusGroup) + self.xScale.bandwidth() / 2 + xShift},${self.height - yShift}  
          ${self.xScale(data[0].focusGroup) + self.xScale.bandwidth() / 2 + xShift - spacer},${self.yScale(data[0].lineValue) - yShift} 
          ${self.xScale(data[0].focusGroup) + self.xScale.bandwidth() / 2 - spacer},${self.yScale(data[0].lineValue)}`)
        .attr("stroke","#ff0070")
        .attr('fill', '#ff007090')
        .attr("stroke-width", 1);
    ...
    topSurfaceContainer
        .append('polygon')
        .attr('points', `${self.xScale(endPoint.focusGroup) + self.xScale.bandwidth() / 2 + spacer},${self.yScale(endPoint.lineValue)} 
          ${self.xScale(endPoint.focusGroup) + self.xScale.bandwidth() / 2 + spacer + xShift},${self.yScale(endPoint.lineValue) - yShift}  
          ${self.xScale(endPoint.focusGroup) + self.xScale.bandwidth() / 2 + xShift},${self.height - yShift} 
          ${self.xScale(endPoint.focusGroup) + self.xScale.bandwidth() / 2},${self.height}`)
        .attr("stroke","#ff0070")
        .attr('fill', '#ff007090')
        .attr("stroke-width", 1);
  ...
  return (
    <div className="chart-container">
      <div ref={containerRef} className="" style={{ transform: `skew(${skewX}deg, ${skewY}deg)`}}></div>
      <div className="flex justify-center items-center gap-20">
        <div className="flex flex-col gap-5">
            <div className="flex items-center gap-3">
              <label htmlFor="z-angle">z-Angle</label>
              <input type="range" id="z-angle" name="z-angle"
                min="0" max="180" onChange={handleZAngleChange} value={`${zAngle}`} />
                <p className="field-value">{zAngle}</p>
            </div>
            <div className="flex items-center gap-3">
              <label htmlFor="z-aspect">z-Aspect</label>
              <input type="range" id="z-aspect" name="z-aspect"
                min="0" max="150" onChange={handleZAspectChange} value={`${zAspect}`} />
        ...
        <div className="flex flex-col gap-5">
            <div className="flex items-center gap-3">
              ...
              <input type="range" id="skew-x" name="skew-x"
                min="-30" max="30" onChange={handleSkewXChange} value={`${skewX}`} />
                <p className="field-value">{skewX}</p>
            </div>
            <div className="flex items-center gap-3">
              <label htmlFor="skew-y">skew-Y</label>
              <input type="range" id="skew-y" name="skew-y"
                min="-30" max="30" onChange={handleSkewYChange} value={`${skewY}`} />
