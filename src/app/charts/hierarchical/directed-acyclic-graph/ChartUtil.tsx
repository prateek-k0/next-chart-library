...
const ChartUtil = ({ data }: { data: DataType }) => {
  ...
  function buildDAG(root: TreeNode, depth: number = 0, visited: { [prop: string]: boolean } = {}) {
    ...
    root.next.forEach((nextId) => {
      ...
      if (visited[nextId] !== true) { // if subtree not traversed already
  ...
  function setLevelOrderData(root: TreeNode = self.rebuiltTreeRoot, visited: { [prop: string]: boolean } = {}) {
    if(self.levelOrderData[root.depth].includes(root.id) === false) self.levelOrderData[root.depth].push(root.id);
  ...
  function renderNodes() {
    self.nodesOrdered = self.levelOrderData.flat().map((nodeId) => self.nodeQMap.get(nodeId) as TreeNode);
    self.chart.selectAll('.node-container').remove();
    ...
    const nodeGroupItem = nodeGroup
      ...
      .attr(
        "transform",
        (d: any) => `translate(${(self.nodeQMap.get(d.id)?.x) ?? 0},${(self.nodeQMap.get(d.id)?.y) ?? 0})`
      );
    nodeGroupItem
      .append("rect")
      .attr("class", "nodeRect")
  ...
  function collapseNode(targetNodeId: string, collapse: boolean, node: TreeNode = self.rebuiltTreeRoot) {
    if(node.id === targetNodeId) {
      const targetNode = self.nodeQMap.get(node.id) as TreeNode;
      targetNode.isCollapsed = collapse;
    } else {
      node.next.forEach((childId) => collapseNode(targetNodeId, collapse, (self.nodeQMap.get(childId) as TreeNode)));
    }
  }

  function renderLinks() {
    self.chart.selectAll('.link-container').remove();
