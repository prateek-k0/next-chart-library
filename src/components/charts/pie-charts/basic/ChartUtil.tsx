...
const ChartUtil = ({ data }: ChartProps) => {
  ...
  const renderFunc = useCallback(() => {
    ...
    const radius =
      Math.min(configRef.current.width, configRef.current.height) / 4 -
    ...
    const legendLine = configRef.current.chart
      .selectAll("line")
      .data(pie(data))
      .enter();
    ...
    const legendCircle1 = configRef.current.chart
      .selectAll("circle")
      .data(pie(data))
      .enter();
    ...
    const labels = configRef.current.chart
      .selectAll("text")
      .data(pie(data))
      .enter();
